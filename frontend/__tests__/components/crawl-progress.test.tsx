...
describe("CrawlProgress", () => {
  it("displays crawled and total counts", () => {
    render(<CrawlProgress progress={defaultProgress} isConnected={true} />);
  ...
  it("shows converting phase in Chinese", () => {
    ...
    render(<CrawlProgress progress={progress} isConnected={true} />);
    expect(screen.getByText(/转换中/)).toBeInTheDocument();
  });

  it("shows disconnected state in Chinese", () => {
    render(<CrawlProgress progress={defaultProgress} isConnected={false} />);
